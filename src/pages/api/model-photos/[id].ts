...
export const PUT: APIRoute = async ({ params, request }) => {
  try {
    ...
    args.push(id);
    await db.execute({ sql: `UPDATE model_photos SET ${fields.join(', ')} WHERE id = ?`, args });
...
export const DELETE: APIRoute = async ({ params }) => {
  try {
    const id = params.id;
    const result = await db.execute({ sql: 'SELECT public_id FROM model_photos WHERE id = ?', args: [id] });
    ...
    await db.execute({ sql: 'DELETE FROM model_photos WHERE id = ?', args: [id] });
