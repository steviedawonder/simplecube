...
export const PUT: APIRoute = async ({ params, request }) => {
  try {
    ...
    args.push(id);
    await db.execute({ sql: `UPDATE backgrounds SET ${fields.join(', ')} WHERE id = ?`, args });
...
export const DELETE: APIRoute = async ({ params }) => {
  try {
    const id = params.id;
    const result = await db.execute({ sql: 'SELECT public_id FROM backgrounds WHERE id = ?', args: [id] });
    ...
    await db.execute({ sql: 'DELETE FROM backgrounds WHERE id = ?', args: [id] });
